 ...
 * Provides methods for calculating gross pay, taxes, and net pay.
 */

const PayrollCalculator = {
  /**
   * Calculates gross pay based on hours worked and pay rate.
   * @param {number} hoursWorked - The number of hours worked.
   * @param {number} payRate - The hourly pay rate.
   * @returns {number} - The calculated gross pay.
   */
  calculateGrossPay: (hoursWorked, payRate) => {
    if (hoursWorked === undefined || payRate === undefined) {
      throw new Error('Hours worked and pay rate are required.');
    }
    ...
    return parseFloat(grossPay.toFixed(2)); // Ensure consistent formatting
  },
  ...
  /**
   * Calculates net pay based on gross pay and total deductions.
   * @param {number} grossPay - The gross pay.
